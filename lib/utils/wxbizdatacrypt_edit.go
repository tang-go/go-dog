...
func (wxCrypt *WxBizDataCrypt) Decrypt(encryptedData string, iv string, isJSON bool) (interface{}, error) {
	...
	if err != nil {
		fmt.Println("解析aes失败", string(aesPlantText))
		return nil, showError{errorCode["IllegalBuffer1"], err}
	}
	...
	if err != nil {
		fmt.Println("解析json失败", string(aesPlantText))
		return nil, showError{errorCode["IllegalBuffer2"], err}
	}
